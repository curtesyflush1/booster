...
describe('PerformanceTracker - Enhanced Tests', () => {
  ...
  describe('performance regression tests', () => {
    ...
    it('should handle high-frequency operations efficiently', async () => {
      ...
      for (let i = 0; i < operations; i++) {
        promises.push(
          tracker.trackOperation('high-freq', async (): Promise<number> => i)
        );
      }
  ...
  describe('timing accuracy', () => {
    it('should provide reasonably accurate timing measurements', async () => {
      ...
      expect(timingMetrics!.totalTime).toBeGreaterThan(expectedDelay - 20);
      expect(timingMetrics!.totalTime).toBeLessThan(expectedDelay + 20);
    });
  });
});