...
export const validateProductIds = (req: Request, res: Response, next: NextFunction): void => {
  ...
  if (productIds && typeof productIds === 'string') {
    ...
    if (invalidIds.length > 0) {
      sendErrorResponse(res, 'INVALID_PRODUCT_IDS', `Invalid product ID format: ${invalidIds.join(', ')}`);
      return;
    }
    
    if (ids.length > 50) {
      sendErrorResponse(res, 'TOO_MANY_PRODUCT_IDS', 'Maximum 50 product IDs allowed');
      return;
    }
  }
...
export const validateSinceDate = (req: Request, res: Response, next: NextFunction): void => {
  ...
  if (since && typeof since === 'string') {
    ...
    if (isNaN(date.getTime())) {
      sendErrorResponse(res, 'INVALID_DATE_FORMAT', 'Invalid date format for since parameter');
      return;
    }
    ...
    if (date > now) {
      sendErrorResponse(res, 'FUTURE_DATE_NOT_ALLOWED', 'Since date cannot be in the future');
      return;
    }
    
    if (date < thirtyDaysAgo) {
      sendErrorResponse(res, 'DATE_TOO_OLD', 'Since date cannot be more than 30 days ago');
      return;
    }
  }
