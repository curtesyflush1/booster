...
async function main() {
  ...
  const [row] = await db('retailers')
    .insert({
      ...
      name: 'Barnes & Noble',
      slug,
      website_url: 'https://www.barnesandnoble.com',
      api_type: 'scraping',
      api_config: JSON.stringify({ base_url: 'https://www.barnesandnoble.com' }),
      is_active: true,
...
main().catch((err) => {
  console.error('Failed to insert Barnes & Noble retailer:', err);
  process.exit(1);
});
